...
/// Constructor
MuonTransformer::MuonTransformer(const ParameterSet& parameterSet, edm::ConsumesCollector& iC)
    : theRPCInTheFit(parameterSet.getParameter<bool>("RefitRPCHits")),
      theGEMInTheFit(parameterSet.getParameter<bool>("RefitGEMHits")),
...
/// Destructor
MuonTransformer::~MuonTransformer() {}

void MuonTransformer::fillPSetDescription(edm::ParameterSetDescription& desc,
  ...
  desc.add<bool>("RefitGEMHits", refitGEMHits);
...
void MuonTransformer::setServices(const EventSetup& setup) {
  const std::string metname = "Reco|TrackingTools|MuonTransformer";
...
vector<Trajectory> MuonTransformer::transform(const reco::TrackRef& track) const { return transform(*track); }

TransientTrackingRecHit::ConstRecHitContainer MuonTransformer::getTransientRecHits(
    ...
    if ((*hit)->isValid()) {
      if ((*hit)->geographicalId().det() == DetId::Tracker) {
        ...
        if ((*hit)->geographicalId().subdetId() == 3 && !theRPCInTheFit) {
          LogTrace("Reco|TrackingTools|MuonTransformer") << "RPC Rec Hit discarged";
          continue;}
	else if ((*hit)->geographicalId().subdetId() == 4 && !theGEMInTheFit) {
          LogTrace("Reco|TrackingTools|MuonTransformer") << "GEM Rec Hit discarged";
          continue;
        }
        ...
        if (theMtdAvailable)
          ...
          throw cms::Exception("MuonTransformer") << "MTD hit encountered but MTD not available!";
      }
    }
  }
...
TransientTrackingRecHit::ConstRecHitContainer MuonTransformer::getTransientRecHits(
										   const reco::TransientTrack& track, int gemcase) const {
    ...
    if ((*hit)->isValid()) {
      if ((*hit)->geographicalId().det() == DetId::Tracker) {
        ...
        if ((*hit)->geographicalId().subdetId() == 3 && !theRPCInTheFit) {
          LogTrace("Reco|TrackingTools|MuonTransformer") << "RPC Rec Hit discarged";
          continue;}
	else if ((*hit)->geographicalId().subdetId() == 4 && gemcase > 0) {
	  GEMDetId gid((*hit)->geographicalId().rawId());
	  if (  (gemcase ==  1 && gid.layer()==2) ||(gemcase ==  2 && gid.layer()==1) ||(gemcase ==  3)){
	    LogTrace("Reco|TrackingTools|MuonTransformer") << "GEM Rec Hit Layer "<<gid.layer()<<" discarged";
	    continue;
	  }
        }
        result.push_back(theMuonRecHitBuilder->build(&**hit));
...
RefitDirection::GeometricalDirection MuonTransformer::checkRecHitsOrdering(
  ...
  LogDebug("Reco|TrackingTools|MuonTransformer") << "Impossible to determine the rechits order" << endl;
  return RefitDirection::undetermined;
}

/// Convert Tracks into Trajectories
vector<Trajectory> MuonTransformer::transform(const reco::Track& newTrack) const {
  const std::string metname = "Reco|TrackingTools|MuonTransformer";
...
/// Convert Tracks into Trajectories
vector<Trajectory> MuonTransformer::muontransform(const reco::Track& newTrack, int gemcase) const {
  const std::string metname = "Reco|TrackingTools|MuonTransformer";

  reco::TransientTrack track(newTrack, magneticField(), trackingGeometry());
  auto recHitsForReFit = getTransientRecHits(track,gemcase);
  return transform(track, recHitsForReFit);
}

/// Convert Tracks into Trajectories with a given set of hits
vector<Trajectory> MuonTransformer::transform(const reco::TransientTrack& track,
                                               TransientTrackingRecHit::ConstRecHitContainer& recHitsForReFit) const {
  const std::string metname = "MuonTransformer";
