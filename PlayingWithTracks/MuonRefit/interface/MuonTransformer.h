#ifndef PlayingWithTracks_MuonRefit_MuonTransformer_H
#define PlayingWithTracks_MuonRefit_MuonTransformer_H

/** \class MuonTransformer based interely from TrackTransformer but adding GEM specs 
 ...
 *  method MUST be called in the code in which the MuonTransformer is used.
 *
...
class MuonTransformer final : public TrackTransformerBase {
public:
  /// Constructor (for modules migrated to ES-consumes)
  explicit MuonTransformer(const edm::ParameterSet&, edm::ConsumesCollector&);
  explicit MuonTransformer(const edm::ParameterSet& parameterSet, edm::ConsumesCollector&& iC)
      : MuonTransformer(parameterSet, iC) {}

  /// Destructor
  ~MuonTransformer() override;
  ...
  /// Convert a reco::Track into Trajectory
  std::vector<Trajectory> muontransform(const reco::Track&, int gemcase) const;
  ...
  /// set the services needed by the MuonTransformer
  ...
  TransientTrackingRecHit::ConstRecHitContainer getTransientRecHits(const reco::TransientTrack& track, int gemcase) const;
